...
public class AppDb
{
    ...
    private async Task InitAsync()
    {
        ...
        var path = Path.Combine(FileSystem.AppDataDirectory, "denemepro_v2.db3");
    ...
    public async Task<(bool Ok, string Error)> RegisterAsync(string email, string password, string phone)
    {
        email = (email ?? "").Trim();
        ...
        if (phone.Length < 5) return (false, "Phone is required.");
        ...
        var user = new User { Email = email, Password = password, Phone = phone };
    ...
    {
        await InitAsync();
        return await _db!.Table<User>().Where(x => x.Id == id).FirstOrDefaultAsync();
    }
